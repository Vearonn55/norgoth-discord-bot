...
from app.models.runtime_events import Campaign, CampaignActivity, CampaignUnsubscribe
...
class CampaignRepository:
    ...
    async def list_by_statuses(self, statuses: list[str]) -> list[Campaign]:
        if not statuses:
            return []
    ...
    async def list_activity(
        ...
        *,
        ...
        query = select(CampaignActivity)
        if campaign_id:
            try:
                uid = UUID(str(campaign_id))
            except ValueError:
                return []
            query = query.where(CampaignActivity.campaign_id == uid)
        query = query.order_by(CampaignActivity.created_at.desc()).limit(limit)
        result = await self._session.execute(query)
        return list(result.scalars().all())

    async def upsert_unsubscribe(self, *, guild_id: str, user_id: str) -> None:
        row = (
            await self._session.execute(
                select(CampaignUnsubscribe)
                .where(
                    CampaignUnsubscribe.guild_id == guild_id,
                    CampaignUnsubscribe.user_id == user_id,
                )
                .with_for_update()
            )
        ).scalar_one_or_none()
        if row is None:
            self._session.add(CampaignUnsubscribe(guild_id=guild_id, user_id=user_id))
            await self._session.commit()

    async def list_unsubscribed_user_ids(self, *, guild_id: str) -> list[str]:
        result = await self._session.execute(
            select(CampaignUnsubscribe.user_id).where(
                CampaignUnsubscribe.guild_id == guild_id
            )
        )
